                ...
                {items.map((item) => (
                  <div key={item.id} className="flex items-center gap-4 bg-white p-4 rounded-lg shadow">
                    ...
                    <div className="flex-1">
                      <div className="font-bold text-lg" style={{ color: '#361c0c' }}>{item.name}</div>
                      {item.variant && (
                    ...
                    <div className="w-24 text-right font-bold" style={{ color: '#361c0c' }}>
                      ₴{(item.quantity * item.price).toFixed(2)}
                    </div>
                    <button onClick={() => removeItem(item.id)} className="ml-2 text-red-600 hover:text-red-700">
